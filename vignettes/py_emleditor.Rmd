...
* Python has a dictionary subclass called an [Ordered Dictionary](https://realpython.com/python-ordereddict/#:~:text=Python's%20OrderedDict%20is%20a%20dict,then%20the%20order%20remains%20unchanged.).
...
```
`reticulate` interprets an `R list` as a `python r.r_metadata.__class__`.
...
```
Where in the python xml document can we find the same value?
...
# Line 118 here concatenates a forward slash: https://github.com/ropensci/emld/blob/master/R/as_emld.R
...
Where in our xml document is `EML::read_eml()` pulling that value for ```r_metadata$`@context`$xml``` from?
