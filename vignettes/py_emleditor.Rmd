...
### Objective 1, task 4: parse input xml to `@context`
...
```
Where in our xml document is `EML::read_eml()` pulling that value? is the value of r_metadata$`@context`$`@vocab`
...
```{python}
import xmltodict # https://www.geeksforgeeks.org/python-program-to-convert-xml-to-dictionary/
with open('C:/Users/cwainright/OneDrive - DOI/Documents/data_projects/2023/20230210_iss135_emleditor/sandbox/2022_NCRN_forest_vegetation_metadata.xml', 'r') as file:
    my_xml = file.read()
my_dict = xmltodict.parse(my_xml)
my_dict["eml:eml"]["@xmlns:eml"]
```
```{python}
